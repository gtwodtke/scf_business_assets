sink("C:\\Users\\Geoff\\Dropbox\\D\\projects\\scf_biz_assets\\programs\\_LOGS\\16_create_table_6_log.txt")

##########################################################
##########################################################
##                                                      ##
## PROGRAM NAME: 16_create_table_6                      ##
## DESCRIPTION:                                         ##
##                                                      ##  
##  trends in actively managed assets                   ##
##                                                      ##
##########################################################
##########################################################
...
for (i in 1:5) {
	...
	scfbiz<-scfmi[which(scfmi$"nminum"==i & scfmi$"nactbizvalpos"==1),]
	dlinmod<-gam(nactbizvalpos~poly(nyear,1,raw=T),family=quasibinomial(link="probit"),data=scfpop,weights=nsampwt)
	dlinmod_coefs<-data.frame(coefficients(dlinmod))
	ylinmod<-gam(nlogactbizval~poly(nyear,1,raw=T),family=gaussian(link="identity"),data=scfbiz,weights=nsampwt)
	...
	for (j in 60:(nboot+59)) {
		boot.dlinmod<-gam(nactbizvalpos~poly(nyear,1,raw=T),family=quasibinomial(link="probit"),data=scfpop,weights=scfpop[,j])
		boot.dlinmod_coefs<-data.frame(coefficients(boot.dlinmod))
		boot.ylinmod<-gam(nlogactbizval~poly(nyear,1,raw=T),family=gaussian(link="identity"),data=scfbiz,weights=scfbiz[,j])
...
scfmibiz<-scfmi[which(scfmi$"nactbizvalpos"==1 & scfmi$"nminum"==1),]
dsmooth_null<-gam(nactbizvalpos~poly(nyear,1,raw=T),family=binomial(link="probit"),data=scfmi,weights=nsampwt)
dsmooth_alt<-gam(nactbizvalpos~s(nyear),family=binomial(link="probit"),gamma=smooth,data=scfmi,weights=nsampwt)
ysmooth_null<-gam(nlogactbizval~poly(nyear,1,raw=T),family=gaussian(link="identity"),data=scfmibiz,weights=nsampwt)
ysmooth_alt<-gam(nlogactbizval~s(nyear),family=gaussian(link="identity"),gamma=smooth,data=scfmibiz,weights=nsampwt)
...
cat("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
cat("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
cat("TABLE 6 - Trends including only Actively \n Managed Business Assets\n")
cat("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
cat("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
cat("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n")
cat("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n")
