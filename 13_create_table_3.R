sink("C:\\Users\\Geoff\\Dropbox\\D\\projects\\scf_biz_assets\\programs\\_LOGS\\13_create_table_3_log.txt")

##########################################################
##########################################################
##                                                      ##
## PROGRAM NAME: 13_create_table_3                      ##
## DESCRIPTION:                                         ##
##                                                      ##  
...
for (i in 1:5) {
	...
	dlinmod<-gam(nbizvalpos~poly(nyear,1,raw=T)*nocc,family=quasibinomial(link="probit"),data=scfpop,weights=nsampwt)
	dlinmod_coefs<-data.frame(coefficients(dlinmod))
	ylinmod<-gam(nlogbizval~poly(nyear,1,raw=T)*nocc,family=gaussian(link="identity"),data=scfbiz,weights=nsampwt)
	ylinmod_coefs<-data.frame(coefficients(ylinmod))
	scfbiz$ehatsq<-residuals(ylinmod,type="response")^2
	esqlindmod<-gam(ehatsq~poly(nyear,1,raw=T)*nocc,family=Gamma(link="log"),data=scfbiz,weights=nsampwt)
	...
	for (j in 60:(nboot+59)) {
		boot.dlinmod<-gam(nbizvalpos~poly(nyear,1,raw=T)*nocc,family=quasibinomial(link="probit"),data=scfpop,weights=scfpop[,j])
		boot.dlinmod_coefs<-data.frame(coefficients(boot.dlinmod))
		boot.ylinmod<-gam(nlogbizval~poly(nyear,1,raw=T)*nocc,family=gaussian(link="identity"),data=scfbiz,weights=scfbiz[,j])
		boot.ylinmod_coefs<-data.frame(coefficients(boot.ylinmod))
		scfbiz$boot.ehatsq<-residuals(boot.ylinmod,type="response")^2
		boot.esqlindmod<-gam(boot.ehatsq~poly(nyear,1,raw=T)*nocc,family=Gamma(link="log"),data=scfbiz,weights=scfbiz[,j])
...
rlabel<-c('intercept','nyear','profmgr','nyear:profmgr')
...
scfmi$nocc<-as.factor(scfmi$nocc)
scfmibiz<-scfmi[which(scfmi$"nbizvalpos"==1 & scfmi$"nminum"==1),]
dsmooth_null<-gam(nbizvalpos~poly(nyear,1,raw=T)*nocc,family=binomial(link="probit"),data=scfmi,weights=nsampwt)
dsmooth_alt<-gam(nbizvalpos~nocc+s(nyear,by=nocc),family=binomial(link="probit"),gamma=smooth,data=scfmi,weights=nsampwt)
ysmooth_null<-gam(nlogbizval~poly(nyear,1,raw=T)*nocc,family=gaussian(link="identity"),data=scfmibiz,weights=nsampwt)
ysmooth_alt<-gam(nlogbizval~nocc+s(nyear,by=nocc),family=gaussian(link="identity"),gamma=smooth,data=scfmibiz,weights=nsampwt)
scfmibiz$ehatsq<-residuals(ysmooth_alt,type="response")^2
esqsmooth_null<-gam(ehatsq~poly(nyear,1,raw=T)*nocc,family=Gamma(link="log"),data=scfmibiz,weights=nsampwt)
esqsmooth_alt<-gam(ehatsq~nocc+s(nyear,by=nocc),family=Gamma(link="log"),gamma=smooth,data=scfmibiz,weights=nsampwt)

### PRINT RESULTS ###
cat("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
cat("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
cat("TABLE 3 - Trends by Occupation\n")
cat("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
cat("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
cat("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n")
cat("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n")
