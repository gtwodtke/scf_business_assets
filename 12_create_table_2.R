sink("C:\\Users\\Geoff\\Dropbox\\D\\projects\\scf_biz_assets\\programs\\_LOGS\\12_create_table_2_log.txt")

##########################################################
##########################################################
##                                                      ##
## PROGRAM NAME: 12_create_table_2                      ##
## DESCRIPTION:                                         ##
##                                                      ##  
##  trends by industry                                  ##
##                                                      ##
...
for (i in 1:5) {
	...
	dlinmod<-gam(nbizvalpos~poly(nyear,1,raw=T)*nindustry,family=quasibinomial(link="probit"),data=scfpop,weights=nsampwt)
	dlinmod_coefs<-data.frame(coefficients(dlinmod))
	ylinmod<-gam(nlogbizval~poly(nyear,1,raw=T)*nindustry,family=gaussian(link="identity"),data=scfbiz,weights=nsampwt)
	ylinmod_coefs<-data.frame(coefficients(ylinmod))
	scfbiz$ehatsq<-residuals(ylinmod,type="response")^2
	esqlindmod<-gam(ehatsq~poly(nyear,1,raw=T)*nindustry,family=Gamma(link="log"),data=scfbiz,weights=nsampwt)
	...
	for (j in 60:(nboot+59)) {
		boot.dlinmod<-gam(nbizvalpos~poly(nyear,1,raw=T)*nindustry,family=quasibinomial(link="probit"),data=scfpop,weights=scfpop[,j])
		boot.dlinmod_coefs<-data.frame(coefficients(boot.dlinmod))
		boot.ylinmod<-gam(nlogbizval~poly(nyear,1,raw=T)*nindustry,family=gaussian(link="identity"),data=scfbiz,weights=scfbiz[,j])
		boot.ylinmod_coefs<-data.frame(coefficients(boot.ylinmod))
		scfbiz$boot.ehatsq<-residuals(boot.ylinmod,type="response")^2
		boot.esqlindmod<-gam(boot.ehatsq~poly(nyear,1,raw=T)*nindustry,family=Gamma(link="log"),data=scfbiz,weights=scfbiz[,j])
...
rlabel<-c('intercept','nyear','goodsind','nyear:goodsind')
...
scfmi<-scfmi[which(scfmi$"nminum"==1),]
scfmi$nindustry<-as.factor(scfmi$nindustry)
scfmibiz<-scfmi[which(scfmi$"nbizvalpos"==1 & scfmi$"nminum"==1),]
dsmooth_null<-gam(nbizvalpos~poly(nyear,1,raw=T)*nindustry,family=binomial(link="probit"),data=scfmi,weights=nsampwt)
dsmooth_alt<-gam(nbizvalpos~nindustry+s(nyear,by=nindustry),family=binomial(link="probit"),gamma=smooth,data=scfmi,weights=nsampwt)
ysmooth_null<-gam(nlogbizval~poly(nyear,1,raw=T)*nindustry,family=gaussian(link="identity"),data=scfmibiz,weights=nsampwt)
ysmooth_alt<-gam(nlogbizval~nindustry+s(nyear,by=nindustry),family=gaussian(link="identity"),gamma=smooth,data=scfmibiz,weights=nsampwt)
scfmibiz$ehatsq<-residuals(ysmooth_alt,type="response")^2
esqsmooth_null<-gam(ehatsq~poly(nyear,1,raw=T)*nindustry,family=Gamma(link="log"),data=scfmibiz,weights=nsampwt)
esqsmooth_alt<-gam(ehatsq~nindustry+s(nyear,by=nindustry),family=Gamma(link="log"),gamma=smooth,data=scfmibiz,weights=nsampwt)

### PRINT RESULTS ###
cat("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
cat("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
cat("TABLE 2 - Trends by Industry\n")
cat("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
cat("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n")
cat("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n")
cat("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n")
